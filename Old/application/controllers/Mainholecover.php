...
class Mainholecover extends Admin_Controller 
{
	...
	*/
	public function index()
	{
		...
		$this->render_template('mainholecover/index', $this->data);		
	}
	...
	public function fetchOrdersData()
	{
		...
		$data = $this->model_labreports->getmainholecoverOrdersData();
		...
		foreach ($data as $key => $value) 
		{
			...
			if(in_array('updateOrder', $this->permission)) {
				$buttons .= ' <a href="'.base_url('mainholecover/update/'.$value['iReportId']).'" class="btn btn-default"><i class="fa fa-pencil"></i></a>';
			}
            ...
            $uid_no = '<a href="'.base_url('mainholecover/update/'.$value['iReportId']).'">'.$value['uid_no'].'</a>';
	 	...
	 	public function fetchOrdersFilterData()
	{
		...
		$data = $this->model_labreports->getmainholecoverOrdersDataFilter($start_date,$end_date);
	...
	public function create()
	{
        ...
        $this->data['cubes'] = $this->model_labreports->getmainholecoverDataByregistration();
		...
		if ($this->form_validation->run() == TRUE) {        	
        	...
        	$iReportId = $this->model_labreports->createmainholecover();
            ...
            for($j=0; $j < $countset; $j++)
            {
              ...
              (
                  ...
                  'hole_type' => $this->input->post('hole_type'),
                  'dia_of_plat' => $this->input->post('dia_of_plat'),
                  'date_of_sample_collection' => $this->input->post('date_of_sample_collection'),
                  'date_of_testing' => $this->input->post('date_of_testing'),
                  'applying_of_load' => $this->input->post('applying_of_load'),
                  'observation' => $this->input->post('observation'),
                  'remark' => $this->input->post('remark'),
                  'set_count' => $countset
                );
                $create = $this->model_labreports->createmainholecoverreport($data);
            }
             
        	if($iReportId) {
        		$this->session->set_flashdata('success', 'Successfully created');
        		redirect('mainholecover', 'refresh');
        	}
        	else 
        	{
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('mainholecover/create/', 'refresh');
        	}
        }
        else {
            // false case
           $this->render_template('mainholecover/create', $this->data);
        }	
	}
    ...
    { 
        ...
        if($uid_no) 
        {
			$data = $this->model_labreports->getmainholecoverDataByregistration($uid_no);
		  	if($data == true) { 
	...
	public function update($id)
	{
        ...
        {        	
        	...
        	$update = $this->model_labreports->updatemainholecover($id);
        	
            $countset = $this->input->post('countset');
             
                    
                    $id = $this->input->post('iMainholeId');
                ...
                $update = $this->model_labreports->updatemainholecoverreport($data,$id);
        	...
        	if($update == true) {
        	 ...
        	 redirect('mainholecover/', 'refresh');
        		...
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('mainholecover/update/'.$id, 'refresh');
        	}
        }
        else {
            // false case
            $result = array();
        	$orders_data = $this->model_labreports->getmainholecoverOrdersData($id);
            ...
            $this->render_template('mainholecover/edit', $this->data);
        }
	}
	...
	*/
	...
	public function remove()
	{
        ...
        $response = array();
        if($iReportId) {
            $delete = $this->model_labreports->mainholecoverremove($iReportId);
            if($delete == true) {
                $response['success'] = true;
	...
	public function printDiv($id)
	{
		...
		if($id) {
			$order_data = $this->model_labreports->getmainholecoverOrdersData($id); 
...
.col-xs-6, .col-xs-4, .col-xs-8
{
  ...
  <table class="table table-striped">
    			        ...
    			        <tr>
        			            <td colspan ="8" style="font-weight:bold;text-align:left;">(A)'.$order_data[0]['hole_type'].'
        			            </td>
    			            </tr>
    			            <tr>
                                <td>1</td>
                                <td>'.$order_data[0]['location'].'</td>
                                <td>'.$order_data[0]['dia_of_plat'].'</td>
                                <td>'.$order_data[0]['date_of_sample_collection'].'</td>
                                <td>'.$order_data[0]['date_of_testing'].'</td>
                                <td>'.$order_data[0]['applying_of_load'].'</td>
                                <td>'.$order_data[0]['observation'].'</td>
                                <td>'.$order_data[0]['remark'].'</td>
        			        </tr>
        		        </tbody>
						</table>
			        </div>
