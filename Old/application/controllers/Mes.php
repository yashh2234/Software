...
class Mes extends Admin_Controller 
{
	public function __construct()
	{
        ...
        $this->data['page_title'] = 'Mes Reports';
	...
	*/
	public function index()
	{
		...
		$this->render_template('mes/index', $this->data);		
	}
	...
	public function fetchOrdersData()
	{
		...
		$data = $this->model_labreports->getmesOrdersData();
		...
		foreach ($data as $key => $value) 
		{
			...
			if(in_array('updateOrder', $this->permission)) {
				$buttons .= ' <a href="'.base_url('mes/update/'.$value['iReportId']).'" class="btn btn-default"><i class="fa fa-pencil"></i></a>';
			}
            ...
            $uid_no = '<a href="'.base_url('mes/update/'.$value['iReportId']).'">'.$value['uid_no'].'</a>';
	 ...
	 public function fetchOrdersFilterData()
	{
		...
		$data = $this->model_labreports->getmesOrdersDataFilter($start_date,$end_date);
	...
	public function create()
	{
        ...
        $this->data['cubes'] = $this->model_labreports->getmesDataByregistration();
		...
		if ($this->form_validation->run() == TRUE) {        	
        	...
        	$iReportId = $this->model_labreports->createmes();
            ...
            for($j=0; $j < $countset; $j++)
            {
                ...
                (
                  ...
                  'los_angles_abrasion_value' => $this->input->post('los_angles_abrasion_value'),
                  'crushing_value' => $this->input->post('crushing_value'),
                  'soundness' => $this->input->post('soundness'),
                  'presence_of_deleterious' => $this->input->post('presence_of_deleterious'), 
                  'organic_impurities' => $this->input->post('organic_impurities'),
                  'specific_gravity' => $this->input->post('specific_gravity'), 
                  'is_sieve_size' => $this->input->post('is_sieve_size_'.$i),
                  'weight_retained' => $this->input->post('weight_retained_'.$i),
                  'weight_retained_in' => $this->input->post('weight_retained_in_'.$i),
                  'cum_weight_retained_in' => $this->input->post('cum_weight_retained_in_'.$i),
                  'passing' => $this->input->post('passing_'.$i),
                  'is_grading_as_per_is_table' => $this->input->post('is_grading_as_per_is_table_'.$i), 
                  'remarks' => $this->input->post('remarks'), 
                  'set_count' => $countset
                );
                $create = $this->model_labreports->createmesreport($data);
            }
             
        	if($iReportId) {
        		$this->session->set_flashdata('success', 'Successfully created');
        		echo'<script>
window.location.href = "'.base_url().'mes";
</script>';
        	...
        	else 
        	{
        		...
        		redirect('mes/create/', 'refresh');
        	}
        }
        else {
            // false case
           $this->render_template('mes/create', $this->data);
        }	
	}
    ...
    public function getClientDetails()
    {
        ...
        if($uid_no) 
        {
			$data = $this->model_labreports->getmesDataByregistration($uid_no);
		  	if($data == true) {
	...
	public function update($id)
	{
        ...
        {        	
        	...
        	$update = $this->model_labreports->updatemes($id);
            ...
            for($j=0; $j < $countset; $j++)
            {
                ...
                $create = $this->model_labreports->updatemesreport($data,$id);
            }
...
window.location.href = "'.base_url().'mes";
        		...
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('mes/update/'.$id, 'refresh');
        	}
        }
        else {
            // false case
            $result = array();
        	$orders_data = $this->model_labreports->getmesOrdersData($id);
            ...
            $this->render_template('mes/edit', $this->data);
        }
	}
	...
	*/
	...
	public function remove()
	{
        ...
        $response = array();
        if($iReportId) {
            $delete = $this->model_labreports->mesremove($iReportId);
            if($delete == true) {
                $response['success'] = true;
	...
	public function printDiv($id)
	{
		...
		if($id) {
			$order_data = $this->model_labreports->getmesOrdersData($id); 
   ...
   <table class="table table-striped">
						...
						<tr>
    			            ...
    			            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Name of Test</th>
    			             <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Test Method</th>
    			        ...
    			        <tr>
    			            <td>1</td>
    			            <td> Impact value</td>
    			            <td style="vertical-align: middle;">IS 2386 part 4:1963</td>	
                        	<td style="vertical-align: bottom;">'.$order_data[0]['impact_value'].'</td>
                        	<td style="vertical-align: bottom;">%</td>
                        	<td style="vertical-align: bottom;">Max.45</td>
	                    </tr>
                           <tr>
    			            <td>2</td>
    			            <td> Los Angles Abrasion value</td>
    			            <td style="vertical-align: middle;">IS 2386 part 4:1963</td>	
                        	<td style="vertical-align: bottom;">'.$order_data[0]['los_angles_abrasion_value'].'</td>
                        	<td style="vertical-align: bottom;">%</td>
                        	<td style="vertical-align: bottom;">Max.30</td>
	                    </tr>
    			           <tr>
    			            <td>3</td>
    			            <td> Crushing Value</td>
    			            <td style="vertical-align: middle;">IS 2386 part 4:1963</td>	
                        	<td style="vertical-align: bottom;">'.$order_data[0]['crushing_value'].'</td>
                        	<td style="vertical-align: bottom;">%</td>
                        	<td style="vertical-align: bottom;">Max.45</td>
	                    </tr>
	                       <tr>
    			            ...
    			            <td> Soundness</td>
    			            <td style="vertical-align: middle;">IS 2386 part 4:1963</td>	
                        	<td style="vertical-align: bottom;">'.$order_data[0]['soundness'].'</td>
                        	<td style="vertical-align: bottom;">%</td>
                        	<td style="vertical-align: bottom;">Max.12</td>
	                    </tr>
	                      <tr>
    			            <td>5</td>
    			            <td> Presence of Deleterious </td>
    			            <td style="vertical-align: middle;">IS 2386 part 4:1963</td>	
                        	<td style="vertical-align: bottom;">'.$order_data[0]['presence_of_deleterious'].'</td>
                        	<td style="vertical-align: bottom;">%</td>
                        	<td style="vertical-align: bottom;">Max.5</td>
	                    </tr>
    			        ...
    			        <tr>
    			            ...
    			            <td style="vertical-align: middle;">IS 2386 part 4:1963</td>	
                        	<td style="vertical-align: bottom;">'.$order_data[0]['organic_impurities'].'</td>
                        	<td style="vertical-align: bottom;">%</td>
                        	<td style="vertical-align: bottom;">--</td>
	                    </tr>
	                     ...
	                     <tr>
    			            ...
    			            <td style="vertical-align: middle;">IS 2386 part 4:1963</td>	
                        	<td style="vertical-align: bottom;">'.$order_data[0]['specific_gravity'].'</td>
                        	<td style="vertical-align: bottom;">%</td>
                        	<td style="vertical-align: bottom;">--</td>
	                    </tr>
	                    </tbody>
	                     ...
	                     ';
                        if($order_data[0]['material_details'] == '12mm (15kg)')
                        {
                          $html.='<h5 style="color:#000!important;text-transform: uppercase;text-align:center;font-size:16px;border: 1px solid;padding: 4px;font-weight: bold;">Gradation of 12.5 Nominal Size Coarse Aggregate ( As per IS 383-2016)							
 </h5>';  
                        }
                        else if($order_data[0]['material_details'] == '15mm (20kg)')
                        {
                          $html.='<h5 style="color:#000!important;text-transform: uppercase;text-align:center;font-size:16px;border: 1px solid;padding: 4px;font-weight: bold;">Gradation of 20 Nominal Size Coarse Aggregate ( As per IS 383-2016)							
 </h5>';    
                        }
                        else if($order_data[0]['material_details'] == '40mm (20kg)')
                        {
                          $html.='<h5 style="color:#000!important;text-transform: uppercase;text-align:center;font-size:16px;border: 1px solid;padding: 4px;font-weight: bold;">Gradation of 40 Nominal Size Coarse Aggregate ( As per IS 383-2016)							
 </h5>';    
                        }
	                   ...
	                   <table class="table table-striped">
    			        ...
    			        <tr>
                            ...
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Weight  retained (5000gm)</th>
                            ...
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">IS Grading As per</br>IS 383 Table-2</th>	
    			        ...
    			        $j = $i+1;
    			          ...
    			          $html.='<tr  style="border:1px solid #000;">
                            ...
                            <td  style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['is_sieve_size'].'</td>
                            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['weight_retained'].'</td>
                            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['weight_retained_in'].'</td>
                            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['cum_weight_retained_in'].'</td>
                            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['passing'].'</td>
                            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['is_grading_as_per_is_table'].'</td>';
                            
                             if($i==0)
                            {
                             
                            $html.='<td rowspan="6" style="border:1px solid #000;color:#000;text-transform: uppercase;vertical-align: middle;">'.$order_data[0]['remarks'].'</td>';
