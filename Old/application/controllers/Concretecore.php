...
class Concretecore extends Admin_Controller 
{
	public function __construct()
	{
        ...
        $this->data['page_title'] = 'Concrete Core Reports';
	...
	*/
	public function index()
	{
		...
		$this->render_template('concretecore/index', $this->data);		
	}
	...
	public function fetchOrdersData()
	{
		...
		$data = $this->model_labreports->getconcretecoreOrdersData();
		...
		foreach ($data as $key => $value) 
		{
			...
			if(in_array('updateOrder', $this->permission)) {
				$buttons .= ' <a href="'.base_url('concretecore/update/'.$value['iReportId']).'" class="btn btn-default"><i class="fa fa-pencil"></i></a>';
			}
            ...
            $uid_no = '<a href="'.base_url('concretecore/update/'.$value['iReportId']).'">'.$value['uid_no'].'</a>';
		...
		public function fetchOrdersFilterData()
	{
		...
		$data = $this->model_labreports->getconcretecoreOrdersDataFilter($start_date,$end_date);
	...
	public function create()
	{
        ...
        $this->data['cubes'] = $this->model_labreports->getconcretecoreDataByregistration();
		...
		if ($this->form_validation->run() == TRUE) {        	
        	...
        	$iReportId = $this->model_labreports->createconcretecore();
            ...
            for($j=0; $j < $countset; $j++)
            {
              ...
              $create = $this->model_labreports->createconcretecorereport($data);
            }
            
        	if($iReportId) {
        		$this->session->set_flashdata('success', 'Successfully created');
        		redirect('concretecore', 'refresh');
        	}
        	else 
        	{
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('concretecore/create/', 'refresh');
        	}
        }
        else {
            // false case
           $this->render_template('concretecore/create', $this->data);
        }	
	}
    ...
    public function getClientDetails()
    {
        ...
        if($uid_no) 
        {
			$data = $this->model_labreports->getconcretecoreDataByregistration($uid_no);
		  	if($data == true) { 
	...
	public function update($id)
	{
        ...
        {        	
        	...
        	$update = $this->model_labreports->updateconcretecore($id);
            ...
            for($j=0; $j < $countset; $j++)
            {
                ...
                $create = $this->model_labreports->updateconcretecorereport($data,$id);
            }
        	...
        	if($update == true) {
        	 ...
        	 redirect('concretecore/', 'refresh');
        		...
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('concretecore/update/'.$id, 'refresh');
        	}
        }
        else {
            // false case
            $result = array();
        	$orders_data = $this->model_labreports->getconcretecoreOrdersData($id);
            ...
            $this->render_template('concretecore/edit', $this->data);
        }
	}
	...
	*/
	public function remove()
	{
        ...
        $response = array();
        if($iReportId) {
            $delete = $this->model_labreports->concretecoreremove($iReportId);
            if($delete == true) {
	...
	public function printDiv($id)
	{
		...
		if($id) {
			$order_data = $this->model_labreports->getconcretecoreOrdersData($id); 
...
.col-xs-6, .col-xs-4, .col-xs-8
{
  ...
  <table class="table table-striped">
						...
						<tr>
                            ...
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Ht. of </br>Core Before</br> Facing (mm)</th>
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Dimension </br>Of Core </br>After Facing
                                <table class="table table-striped"><tr><td style="border:1px solid #000;color:#000;text-transform: uppercase;">Dia</br>(mm)</td><td style="border:1px solid #000;color:#000;text-transform: uppercase;">Height</br>(mm)</td></tr></table>
                            </th>
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Cross </br>Sectional </br>Area mm2</th>
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Correction </br>Factor</th>
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Age Of </br> Specimen</th>
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Crushing </br>Load </br>In KN</th>
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Measured </br>Comp. </br>Strength</th>
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Corrected </br>Comp. </br>Strength</th>
                            <th style="border:1px solid #000;color:#000;text-transform: uppercase;">Equivalent </br>Cube </br>Strength</th>
                        </tr>';
        			     ...
        			     $i = $p+1;
                            ...
                            $html.='<tr>
        			            <td>'.$i.'</td>
        			            <td>'.$order_data[$p]['ht_core_before_facing'].'</td>
        			            <td><table class="table table-striped"><tr>
        			            <td>'.$order_data[$p]['dimension_core_facing_dia'].'</td>
        			            <td>'.$order_data[$p]['dimension_core_facing_height'].'</td>
        			            </tr></table></td>
        			            <td>'.$order_data[$p]['core_sectional_area'].'</td>
        			            <td>'.$order_data[$p]['correction_factor'].'</td>';
        			            if($i == 1){
        			             $html.='<td rowspan="'.$rowcount.'" style="vertical-align:middle;">'.$order_data[$p]['age_of_specimen'].'</td>';
        			            }
        			             $html.='<td>'.$order_data[$p]['crushing_load'].'</td>
        			            <td>'.$order_data[$p]['measured_comp_strength'].'</td>
        			            <td>'.$order_data[$p]['measured_comp_strength'].'</td>
        			            <td>'.$order_data[$p]['equivalent_cube_strength'].'</td>
        			        </tr>';
                             }
