...
class Water extends Admin_Controller 
{
	...
	*/
	public function index()
	{
		...
		$this->render_template('water/index', $this->data);		
	}
	...
	public function fetchOrdersData()
	{
		...
		$data = $this->model_labreports->getwaterOrdersData();
		...
		foreach ($data as $key => $value) 
		{
			...
			if(in_array('updateOrder', $this->permission)) {
				$buttons .= ' <a href="'.base_url('water/update/'.$value['iReportId']).'" class="btn btn-default"><i class="fa fa-pencil"></i></a>';
			}
            ...
            $uid_no = '<a href="'.base_url('water/update/'.$value['iReportId']).'">'.$value['uid_no'].'</a>';
	 ...
	 public function fetchOrdersFilterData()
	{
		...
		$data = $this->model_labreports->getwaterOrdersDataFilter($start_date,$end_date);
	...
	public function create()
	{
        ...
        $this->data['cubes'] = $this->model_labreports->getwaterDataByregistration();
		...
		if ($this->form_validation->run() == TRUE) {        	
        	...
        	$iReportId = $this->model_labreports->createwater();
            ...
            for($j=0; $j < $countset; $j++)
            {
               ...
               (
                  ...
                  'specification' => $this->input->post('specification_'.$i),
                  'set_count' => $countset
                );
                $create = $this->model_labreports->createwaterreport($data);
            }
             
        	if($iReportId) {
        		$this->session->set_flashdata('success', 'Successfully created');
        		echo'<script>
window.location.href = "'.base_url().'water";
</script>';
        	...
        	else 
        	{
        		...
        		redirect('water/create/', 'refresh');
        	}
        }
        else {
            // false case
           $this->render_template('water/create', $this->data);
        }	
	}
    ...
    public function getClientDetails()
    {
        ...
        if($uid_no) 
        {
			$data = $this->model_labreports->getwaterDataByregistration($uid_no);
		  	if($data == true) {
	...
	public function update($id)
	{
        ...
        {        	
        	...
        	$update = $this->model_labreports->updatewater($id);
            ...
            for($j=0; $j < $countset; $j++)
            {
                ...
                $create = $this->model_labreports->updatewaterreport($data,$id);
            }
...
window.location.href = "'.base_url().'water";
        		...
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('water/update/'.$id, 'refresh');
        	}
        }
        else {
            // false case
            $result = array();
        	$orders_data = $this->model_labreports->getwaterOrdersData($id);
            ...
            $this->render_template('water/edit', $this->data);
        }
	}
	...
	*/
	...
	public function remove()
	{
        ...
        $response = array();
        if($iReportId) {
            $delete = $this->model_labreports->waterremove($iReportId);
            if($delete == true) {
                $response['success'] = true;
	...
	public function printDiv($id)
	{
		...
		if($id) {
			$order_data = $this->model_labreports->getwaterOrdersData($id); 
    			        ...
    			        <tbody>
            			 ...
            			 $i = $j+1;
            			  ...
            			  $html.='<tr>
                                    ...
                                    <td>'.$order_data[$j]['mathod_of_test'].'</td>
                                    <td>'.$order_data[$j]['result'].'</td>
                                    <td>'.$order_data[$j]['unit'].'</td>
                                    <td>'.$order_data[$j]['specification'].'</td>
    			                </tr>';
        			          } 
