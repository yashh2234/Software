...
class Bricks extends Admin_Controller 
{
	public function __construct()
	{
        ...
        $this->data['page_title'] = 'Bricks Reports';
	...
	*/
	public function index()
	{
		...
		$this->render_template('bricks/index', $this->data);		
	}
	...
	public function fetchOrdersData()
	{
		...
		$data = $this->model_labreports->getbricksOrdersData();
		...
		foreach ($data as $key => $value) 
		{
			...
			if(in_array('updateOrder', $this->permission)) {
				$buttons .= ' <a href="'.base_url('bricks/update/'.$value['iReportId']).'" class="btn btn-default"><i class="fa fa-pencil"></i></a>';
			}
            ...
            $uid_no = '<a href="'.base_url('bricks/update/'.$value['iReportId']).'">'.$value['uid_no'].'</a>';
	 ...
	 public function fetchOrdersFilterData()
	{
		...
		$data = $this->model_labreports->getbricksOrdersDataFilter($start_date,$end_date);
	...
	public function create()
	{
        ...
        $this->data['cubes'] = $this->model_labreports->getbricksDataByregistration();
		...
		if ($this->form_validation->run() == TRUE) {        	
        	...
        	$iReportId = $this->model_labreports->createbricks();
            ...
            for($j=0; $j < $countset; $j++)
            {
                ...
                (
        	      'iReportId' => $iReportId,
                  'uid_no' =>  $uid_no,
                  'dimension_length' => $this->input->post('length'),
                  'dimension_width' => $this->input->post('width'),
                  'dimension_height' => $this->input->post('height'),
                  'water_absorption' => $this->input->post('water_absorption'),
                  'efflorescence' => $this->input->post('efflorescence'), 
                  'compressive_strength_main' => $this->input->post('compressive_strength'),
                  'length' => $this->input->post('length_'.$i),
                  'width' => $this->input->post('width_'.$i),
                  'load' => $this->input->post('load_'.$i),
                  'compressive_strength' => $this->input->post('compressive_strength_'.$i),
                  'avg_compressive_strength' => $this->input->post('avg_compressive_strength'),
                  'limit' => $this->input->post('compressive_strength_limit'), 
                  'set_count' => $countset
                );
                $create = $this->model_labreports->createbricksreport($data);
            }
             
        	if($iReportId) {
        		$this->session->set_flashdata('success', 'Successfully created');
        		echo'<script>
window.location.href = "'.base_url().'bricks";
</script>';
        		//redirect('bricks', 'refresh');
        	}
        	else 
        	{
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('bricks/create/', 'refresh');
        	}
        }
        else {
            // false case
           $this->render_template('bricks/create', $this->data);
        }	
	}
    ...
    public function getClientDetails()
    {
        ...
        if($uid_no) 
        {
			$data = $this->model_labreports->getbricksDataByregistration($uid_no);
		  	if($data == true) {
	...
	public function update($id)
	{
        ...
        {        	
        	...
        	$update = $this->model_labreports->updatebricks($id);
        	
            echo $countset = $this->input->post('countset');
            for($j=0; $j < $countset; $j++)
            {
                    $i = $j+1;
                    
                    $id = $this->input->post('iBricksId_'.$i);
                ...
                $create = $this->model_labreports->updatebricksreport($data,$id);
            }
...
window.location.href = "'.base_url().'bricks";
        		...
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('bricks/update/'.$id, 'refresh');
        	}
        }
        else {
            // false case
            $result = array();
        	$orders_data = $this->model_labreports->getbricksOrdersData($id);
            ...
            $this->render_template('bricks/edit', $this->data);
        }
	}
	...
	*/
	...
	public function remove()
	{
        ...
        $response = array();
        if($iReportId) {
            $delete = $this->model_labreports->bricksremove($iReportId);
            if($delete == true) {
                $response['success'] = true;
	...
	public function printDiv($id)
	{
		...
		if($id) {
			$order_data = $this->model_labreports->getbricksOrdersData($id); 
...
.col-xs-6, .col-xs-4, .col-xs-8
{
		        ...
		        <div class="row" style="border:2px solid;">
                 ...
                 <div class="col-xs-12 col-sm-12" style="border: 1px solid;width: 99.3%;margin-left: 3px;">
                    ...
                    <div class="col-xs-12 col-sm-12"  style="border-bottom: 1px solid;width: 99.3%;border-left: 1px solid;border-right: 1px solid;margin-left: 3px;">
                        ...
                        <div class="col-xs-12 col-sm-12">
                        <h5 style="color:#000!important;text-transform: uppercase;text-align:center;font-size:16px;border: 1px solid;padding: 4px;font-weight: bold;">The Below Sample Confrim as per IS Specification 456-2000 {Clause:5:4} </h5>
                       </div>
    ...
    <table class="table table-striped">
    			        ...
    			        <tr>
                        	...
                        	<td rowspan="2" style="vertical-align: bottom;">'.$order_data[0]['dimension_length'].'</td>
                         ...
                         <tr>
    			            ...
    			            <td>(ii) Width</td>
    			            <td>'.$order_data[0]['dimension_width'].'</td>
    			            <td>mm</td>
    			            <td>2200±40</td>
                        </tr>
                         <tr>
    			            ...
    			            <td>(iii) Height</td>
    			            <td>'.$order_data[0]['dimension_height'].'</td>
    			            <td>mm</td>
    			            <td>1400±40</td>
                        </tr>
        			      <tr>
                            ...
                            <td>'.$order_data[0]['water_absorption'].'</td>
                            <td>%</td>
                            <td>20 Max.</td>
    			       </tr> 
    			          <tr>
                            ...
                            <td>'.$order_data[0]['efflorescence'].'</td>
                            <td>-</td>
                            <td>Moderate Max.</td>
    			       </tr> 
    			           <tr>
                            ...
                            <td>'.$order_data[0]['compressive_strength_main'].'</td>
                            <td>N/mm2</td>
                            <td>'.$order_data[0]['limit'].'</td>
    			       </tr> 
    			        <tr>
    			          ...
    			          $html.='<tr>';
    			             ...
    			             if($i == 0)
        			       {
    			            $html.='<td rowspan="5" style="border:1px solid #000;color:#000;text-transform: uppercase;"></td>';
    			             } 
    			            $html.='<td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['length'].'</td>
    			            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['width'].'</td>
                            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['load'].'</td>
                            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['compressive_strength'].'</td>';
                       
    			             if($i == 0)
        			            {
        			         
        			              $html.='<td rowspan="5" style="verborder:1px solid #000;color:#000;text-transform: uppercase;vertical-align: middle;">'.$order_data[0]['avg_compressive_strength'].'</td>';
