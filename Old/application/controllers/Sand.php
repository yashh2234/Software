...
class Sand extends Admin_Controller 
{
	public function __construct()
	{
        ...
        $this->data['page_title'] = 'Sand Reports';
	...
	*/
	public function index()
	{
		...
		$this->render_template('sand/index', $this->data);		
	}
	...
	public function fetchOrdersData()
	{
		...
		$data = $this->model_labreports->getsandOrdersData();
		...
		foreach ($data as $key => $value) 
		{
			...
			if(in_array('updateOrder', $this->permission)) {
				$buttons .= ' <a href="'.base_url('sand/update/'.$value['iReportId']).'" class="btn btn-default"><i class="fa fa-pencil"></i></a>';
			}
            ...
            $uid_no = '<a href="'.base_url('sand/update/'.$value['iReportId']).'">'.$value['uid_no'].'</a>';
	 ...
	 public function fetchOrdersFilterData()
	{
		...
		$data = $this->model_labreports->getsandOrdersDataFilter($start_date,$end_date);
	...
	public function create()
	{
        ...
        $this->data['cubes'] = $this->model_labreports->getsandDataByregistration();
		...
		if ($this->form_validation->run() == TRUE) {        	
        	...
        	$iReportId = $this->model_labreports->createsand();
            ...
            for($j=0; $j < $countset; $j++)
            {
                ...
                (
                  ...
                  'weight_retained_in_gm' => $this->input->post('weight_retained_in_gm_'.$i),
                  'weight_retained_in_perc' => $this->input->post('weight_retained_in_perc_'.$i),
                  'cum_weight_retained_in_perc' => $this->input->post('cum_weight_retained_in_perc_'.$i),
                  'passing' => $this->input->post('passing_'.$i),
                  'is_grading_as_per_is' => $this->input->post('is_grading_as_per_is_'.$i), 
                  'remarks' => $this->input->post('remarks'), 
                  'set_count' => $countset
                );
                $create = $this->model_labreports->createsandreport($data);
            }
             
        	if($iReportId) {
        		$this->session->set_flashdata('success', 'Successfully created');
        		echo'<script>
window.location.href = "'.base_url().'sand";
</script>';
        		//redirect('sand', 'refresh');
        	}
        	else 
        	{
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('sand/create/', 'refresh');
        	}
        }
        else {
            // false case
           $this->render_template('sand/create', $this->data);
        }	
	}
    ...
    public function getClientDetails()
    {
        ...
        if($uid_no) 
        {
			$data = $this->model_labreports->getsandDataByregistration($uid_no);
		  	if($data == true) {
	...
	public function update($id)
	{
        ...
        {        	
        	...
        	$update = $this->model_labreports->updatesand($id);
            ...
            for($j=0; $j < $countset; $j++)
            {
                ...
                $create = $this->model_labreports->updatesandreport($data,$id);
            }
...
window.location.href = "'.base_url().'sand";
        		...
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('sand/update/'.$id, 'refresh');
        	}
        }
        else {
            // false case
            $result = array();
        	$orders_data = $this->model_labreports->getsandOrdersData($id);
            ...
            $this->render_template('sand/edit', $this->data);
        }
	}
	...
	*/
	...
	public function remove()
	{
        ...
        $response = array();
        if($iReportId) {
            $delete = $this->model_labreports->sandremove($iReportId);
            if($delete == true) {
                $response['success'] = true;
	...
	public function printDiv($id)
	{
		...
		if($id) {
			$order_data = $this->model_labreports->getsandOrdersData($id); 
		        ...
		        <div class="row" style="border:2px solid;">
                 ...
                 <div class="col-xs-12 col-sm-12" style="border: 1px solid;width: 99.3%;margin-left: 3px;">
                    ...
                    <div class="col-xs-12 col-sm-12"  style="border-bottom: 1px solid;width: 99.3%;border-left: 1px solid;border-right: 1px solid;margin-left: 3px;">
                        ...
                        <div class="col-xs-12 col-sm-12">
                        <h5 style="color:#000!important;text-transform: uppercase;text-align:center;font-size:16px;border: 1px solid;padding: 4px;font-weight: bold;">Gradation of Fine Aggregate ( As per IS 383-2016)</h5>
                       </div>
    			        ...
    			        for($i=0;$i<$countset;$i++){  
    			          $html.='<tr>';
    			             ...
    			             $j = $i+1;
    			              ...
    			              $html.='<td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['is_sieve_size'].'</td>
    			            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['weight_retained_in_gm'].'</td>
    			            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['weight_retained_in_perc'].'</td>
    			            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['cum_weight_retained_in_perc'].'</td>
                            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['passing'].'</td>
                            <td style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['is_grading_as_per_is'].'</td>';
                             
    			             if($i == 0)
        			            {
        			            
        			              $html.='<td rowspan="7" style="border:1px solid #000;color:#000;text-transform: uppercase;">'.$order_data[$i]['remarks'].'</td>';
