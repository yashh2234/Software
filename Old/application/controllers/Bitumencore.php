...
class Bitumencore extends Admin_Controller 
{
	...
	*/
	public function index()
	{
		...
		$this->render_template('bitumencore/index', $this->data);		
	}
	...
	public function fetchOrdersData()
	{
		...
		$data = $this->model_labreports->getbitumencoreOrdersData();
		...
		foreach ($data as $key => $value) 
		{
			...
			if(in_array('updateOrder', $this->permission)) {
				$buttons .= ' <a href="'.base_url('bitumencore/update/'.$value['iReportId']).'" class="btn btn-default"><i class="fa fa-pencil"></i></a>';
			}
            ...
            $uid_no = '<a href="'.base_url('bitumencore/update/'.$value['iReportId']).'">'.$value['uid_no'].'</a>';
	 ...
	 public function fetchOrdersFilterData()
	{
		...
		$data = $this->model_labreports->getbitumencoreOrdersDataFilter($start_date,$end_date);
	...
	public function create()
	{
        ...
        $this->data['cubes'] = $this->model_labreports->getbitumencoreDataByregistration();
		...
		if ($this->form_validation->run() == TRUE) {        	
        	...
        	$iReportId = $this->model_labreports->createbitumencore();
            ...
            for($j=0; $j < $countset; $j++)
            {
               ...
               (
                      ...
                      'thickness' => $this->input->post('thickness_'.$i),
                      'date_of_sampling' => $this->input->post('date_of_sampling_'.$i),
                      'mathod_of_test' => $this->input->post('method_of_testing_'.$i),
                      'density' => $this->input->post('density_'.$i),
                      'result' => $this->input->post('result_'.$i),
                      'set_count' => $countset
                );
                $create = $this->model_labreports->createbitumencorereport($data);
            }
             
        	if($iReportId) {
        		$this->session->set_flashdata('success', 'Successfully created');
        		redirect('bitumencore', 'refresh');
        	}
        	else 
        	{
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('bitumencore/create/', 'refresh');
        	}
        }
        else {
            // false case
           $this->render_template('bitumencore/create', $this->data);
        }	
	}
    ...
    public function getClientDetails()
    {
        ...
        if($uid_no) 
        {
			$data = $this->model_labreports->getbitumencoreDataByregistration($uid_no);
		  	if($data == true) {
	...
	public function update($id)
	{
        ...
        {        	
        	...
        	$update = $this->model_labreports->updatebitumencore($id);
            ...
            for($j=0; $j < $countset; $j++)
            {
                    ...
                    $id = $this->input->post('iBitumenCId_'.$i);
                ...
                $create = $this->model_labreports->updatebitumencorereport($data,$id);
            }
        	...
        	if($update == true) {
        		...
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('bitumencore/update/'.$id, 'refresh');
        	}
        }
        else {
            // false case
            $result = array();
        	$orders_data = $this->model_labreports->getbitumencoreOrdersData($id);
            ...
            $this->render_template('bitumencore/edit', $this->data);
        }
	}
	...
	*/
	...
	public function remove()
	{
        ...
        $response = array();
        if($iReportId) {
            $delete = $this->model_labreports->bitumencoreremove($iReportId);
            if($delete == true) {
                $response['success'] = true;
	...
	public function printDiv($id)
	{
		...
		if($id) {
			$order_data = $this->model_labreports->getbitumencoreOrdersData($id); 
			...
			<body>
		       <div class="container">
		        <div class="row" style="margin-bottom:20px;">
		            ...
		            <img src="'.base_url().'assets/images/report_header.jpeg" class="img-responsive" style="width: 1170px;height: 130px;">
		            </div>
		        </div>
  ...
  <table class="table table-striped">
    			       ...
    			       $rowcount = $order_data[0]['set_count'];
    			        ...
    			        $i = $j+1;
    			         $html.='<tr>
        			            ...
        			            $html.='<td>'.$order_data[$j]["density"].'</td>
				    ...
				    <div class="col-xs-12">
				     ...
				     <div class="row" style="margin-top:20px;">
    				    <div class="col-xs-12">
    			            <img src="'.base_url().'assets/images/report_footer.jpeg" class="img-responsive" style="width: 1170px;height: 50px;">
    			         </div>
