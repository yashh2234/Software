...
import { ArrowLeft, Circle, ArrowRight, Clock, User, Check, Send, Activity, Ban, Box, Briefcase, Calendar, AlertTriangle, RotateCcw } from 'lucide-react'
import { request } from '../lib/api'
import type { Job, WorkflowTransition, Sample } from '../lib/types'
    ...
    <div>
      ...
      <div className="surface" style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, padding: '0 20px 16px' }}>
          {job.client ? (
            <div>
              <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>Client</span>
              <strong>{job.client.company_name}</strong>
            </div>
          ) : null}
          {job.assigned_user ? (
            <div>
              <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>Assigned To</span>
              <strong style={{ display: 'flex', alignItems: 'center', gap: 4 }}><User size={14} /> {job.assigned_user.name}</strong>
            </div>
          ) : null}
          {job.started_at ? (
            <div>
              <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>Started</span>
              <strong>{new Date(job.started_at).toLocaleDateString()}</strong>
            </div>
          ) : null}
          {job.completed_at ? (
            <div>
              <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>Completed</span>
              <strong>{new Date(job.completed_at).toLocaleDateString()}</strong>
            </div>
          ) : null}
                ...
                return (
                  <div key={stage.id} style={{ display: 'flex', alignItems: 'center', flexShrink: 0 }}>
                    <div style={{
                      ...
                      fontSize: '0.75rem', color: isCurrent || isCompleted ? stage.color : '#9ca3af',
                      whiteSpace: 'nowrap',
                    }}>
                    ...
                    {idx < workflowStages.length - 1 ? (
                      <ArrowRight size={12} style={{ margin: '0 3px', color: '#d1d5db', flexShrink: 0 }} />
                    ) : null}
                  </div>
                )
              })}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'start' }}>
        ...
        <div>
          ...
          <SectionCard title="Timeline" icon={Activity}>
            {timeline.length === 0 ? (
              <p style={{ color: '#65737d' }}>No timeline entries yet.</p>
            ) : (
                  ...
                  <div key={entry.id} style={{ position: 'relative', paddingBottom: 14 }}>
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <span style={{ fontSize: '0.7rem', color: '#65737d', whiteSpace: 'nowrap' }}>
          ...
          <SectionCard title="Samples" icon={Box}>
            {samples.length === 0 ? (
              <p style={{ color: '#65737d' }}>No samples registered for this job.</p>
            ) : (
              <div style={{ display: 'grid', gap: 8 }}>
                {samples.map((s) => (
                  <div key={s.id} style={{ padding: '10px 14px', border: '1px solid #e8eef1', borderRadius: 8 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <strong>{s.sample_name || s.sample_type || 'Sample'}</strong>
                      {s.quantity ? <span className="sync-pill">{s.quantity}{s.unit ? ` ${s.unit}` : ''}</span> : null}
                    </div>
                    {s.condition ? <span style={{ fontSize: '0.78rem', color: '#65737d' }}>Condition: {s.condition}</span> : null}
                    {s.remarks ? <p style={{ fontSize: '0.78rem', color: '#4a5b66', marginTop: 4 }}>{s.remarks}</p> : null}
                  </div>
                ))}
              </div>
            )}
          ...
          <SectionCard title="Actions" icon={Send}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, padding: 12, background: '#f9fafb', borderRadius: 8 }}>
              ...
              <input
                ...
                placeholder="User ID"
                style={{ width: 100, padding: '6px 10px', borderRadius: 6, border: '1px solid #dfe6ea', fontSize: '0.82rem' }}
              />
              ...
              {job.status !== 'cancelled' ? (
                <button className="ghost-button" onClick={() => void handleCancel()} type="button" style={{ color: '#ef4444' }}><Ban size={14} /> Cancel</button>
              ) : null}
            </div>
        ...
        <div>
          ...
          <SectionCard title="Job Details" icon={Briefcase}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>UID</span>
                <span style={{ fontWeight: 600, fontSize: '0.88rem' }}>{job.uid_no}</span>
              </div>
              <div>
                <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>Workflow Template</span>
                <span style={{ fontSize: '0.88rem' }}>{job.workflow_template?.name ?? '-'}</span>
              </div>
              <div>
                <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>Current Stage</span>
                <span style={{ fontWeight: 600, fontSize: '0.88rem' }}>{job.current_stage?.name ?? 'Not started'}</span>
              </div>
              <div>
                <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>Client</span>
                <span style={{ fontSize: '0.88rem' }}>{job.client?.company_name ?? '-'}</span>
              </div>
              <div>
                <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>Created</span>
                <span style={{ fontSize: '0.88rem' }}>{job.created_at ? new Date(job.created_at).toLocaleString() : '-'}</span>
              </div>
              <div>
                <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>Due Date</span>
                <span style={{ fontSize: '0.88rem' }}>{job.due_at ? new Date(job.due_at).toLocaleString() : 'Not set'}</span>
              </div>
            </div>
            {job.description ? (
              <div style={{ marginTop: 12 }}>
                <span style={{ fontSize: '0.72rem', color: '#65737d', display: 'block' }}>Description</span>
                <p style={{ margin: '4px 0', fontSize: '0.85rem' }}>{job.description}</p>
              </div>
            ) : null}
          </SectionCard>
